...
public final class Cli implements Runnable {
    ...
    private int chooseMenuOption(List<MenuOption> options) {
        System.out.print("Enter a menu option a press ENTER");
    ...
    private void doCustomerAction(int subMenuChoice) {
        if (subMenuChoice == 1) {
            ...
            throw new UnsupportedOperationException("There are only 4 customer menu options, this cannot happpen.");
        }
    }
    ...
    private void doOrderList() {
        final Collection<Order> orders = warehouse.getOrders();
        int maxIdWidth = 0;
